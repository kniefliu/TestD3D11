...
const TCHAR *strWndClassName = L"D3D11ShadowWnd";

HINSTANCE CWndShadow::s_hInstance = (HINSTANCE)INVALID_HANDLE_VALUE;

std::vector<HWNDSHADOW> CWndShadow::s_ShadowArray;
...
{
	for (auto i = s_ShadowArray.begin(); i < s_ShadowArray.end(); ++i)
	{
        HWND parentHwnd = (HWND)i->hWnd;
        if (parentHwnd == hWnd) return i->pWndShadow;
    }
    return NULL;
}

void CWndShadow::RemoveShadowWindow(HWND hWnd)
{
    for (auto i = s_ShadowArray.begin(); i < s_ShadowArray.end(); ++i)
    {
		HWND parentHwnd = (HWND)i->hWnd;
		if (parentHwnd == hWnd) {
			s_ShadowArray.erase(i);
			return;
		}
    }
}
